...
def translate_file(path: pathlib.Path, language: str, all_: bool) -> None:
    # إذا كانت اللغة هي العربية، تجاوز الترجمة التلقائية
    if language == "ar":
        print(f"Skipping auto-translation for Arabic ({language})")
        return
...
def translate_phrase(text: str, language: str) -> str:
    response = requests.post(
        ...
        json={
            "model": OPENAI_MODEL,
            "messages": [
                {"role": "system", "content": OPENAI_PROMPT.format(language=language)},
...
def main():
    ...
    arg_parser.add_argument("-t", "--all-translations", action="store_true", default=False,
                            help="Translate all sections. (Default: only unfinished)")
